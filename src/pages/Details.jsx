      ...
      {CATEGORIES.map((category) => {
        ...
        return (
          <div key={category.value} id={`category-${category.value}`} className="mb-6 scroll-mt-4">
            ...
            {guides.length === 0 ? (
              <p className="rounded-2xl border-2 border-background-200 bg-background-50 p-4 text-sm text-foreground-400">
                {t('details.noGuides')}
              </p>
            ) : (
              <div className="space-y-3">
                {guides.map((guide) => (
                  <Link
                    ...
                    className="flex items-center gap-3 rounded-2xl border-2 border-background-200 bg-background-50 p-4 transition-colors hover:border-primary-300 hover:bg-primary-50"
                  >
